...
const Thread = ({ userName, threadId }) => {
  ...
  return (
    <div className="flex bg-[#1A1A38] text-white min-h-screen">
         {/* Sidebar */}
      {/* Sidebar - Fixed width */}
...
<div className="flex-grow p-4 space-y-4">
  <div className="relative flex items-center justify-between mb-6">
    <div className="flex-grow absolute left-20"> 
      <SearchComponent />
    </div>

    {/* Profile Circle on the right side */}
    <div className="absolute right-5 top-2 w-20 h-20 rounded-full bg-gray-400 flex items-center justify-center text-white">

    </div>
  </div>

        {/* Header and Content */}
        <div className="mb-10 h-[500px] p-8 rounded-xl bg-[#1A1A38] shadow-xl">
          <div className="text-gray-400 text-base flex items-center gap-2 mb-2 left-10">
          <div className="w-4 h-4 bg-white rounded-full mt-10"></div>
            <span className="text-gray-300 text-md font-medium font-roboto mt-10">{threads.userr.userName} • Posted 5 days ago</span>
          </div>
          {threads.map(thread => ( <div key={thread.threadId} className="p-4 border rounded shadow">
            <h1 className="text-4xl font-black mb-6 text-white leading-tight font-roboto">
            {thread.threadName}
            </h1>
            <p className="text-2xl font-black mb-6 text-white leading-tight font-roboto">{thread.description}</p>
             </div>))}
        ...
        <div className="border-t border-gray-600 pt-8">
          <h2 className="flex items-center space-x-3 text-white text-3xl font-bold mb-8 font-roboto">
            <svg
              className="w-7 h-7"
              fill="none"
              stroke="currentColor"
            ...
            >
              <path
                ...
                strokeWidth={2}
                d="M17 8h2a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V10a2 2 0 012-2h2m10-4H7a2 2 0 00-2 2v4h14V6a2 2 0 00-2-2z"
              />
            </svg>
      ...
      {/* New Comment Input */}
<div className="border border-gray-600 rounded-xl p-6 mb-8 bg-[#1a1a3d] font-roboto">
  <textarea
    className="w-full bg-[#1A1A38] border-2 border-white outline-none resize-none text-white text-lg rounded-lg p-4 focus:border-white font-roboto"
    rows="5"
  ...
  <div className="flex justify-end mt-4">
    <button
      ...
      className="bg-white text-black px-6 py-2 rounded-full font-bold text-lg hover:bg-gray-100 transition duration-200 font-roboto"
    >
      Post Comment
    </button>
  </div>
